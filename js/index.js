function Index() {
  this.memberId = "583ba1dde7374718d0c4f773";
...
Index.prototype = {
  ...
  init: function() {
    var that = this;
    that.videoContent = document.getElementById('video');
    $('.begin').on('click', function() {
      $(this).hide();
      that.videoContent.play();
      that.videoContent.addEventListener('timeupdate', pauseVideo, false);
      that.videoContent.addEventListener('ended', function() {
        location.href = 'select.html?memberId=' + that.memberId
      }, false);
    })

    function pauseVideo() {
      if (that.videoContent.currentTime >= 25) {
        that.videoContent.onclick = function() {
          that.videoContent.pause();
          location.href = 'select.html?memberId=' + that.memberId
        };
      }
    }
